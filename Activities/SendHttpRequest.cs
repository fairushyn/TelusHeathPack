...
namespace TelusHeathPack.Activities
{
    ...
    public class SendHttpRequest : Activity
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var uri = await _expressionEvaluator.EvaluateAsync(Url, workflowContext, cancellationToken);
            var request = new HttpRequestMessage(new HttpMethod(Method), uri);

            var authorizationHeaderValue = await _expressionEvaluator.EvaluateAsync(
                Authorization,
                workflowContext,
                cancellationToken
            );
            ...
            {
                var body = await _expressionEvaluator.EvaluateAsync(Content, workflowContext, cancellationToken);
        ...
        private async Task<IHeaderDictionary> ParseRequestHeadersAsync(
            ...
            CancellationToken cancellationToken)
        {
            var headersText = await _expressionEvaluator.EvaluateAsync(
                RequestHeaders,
                workflowContext,
                cancellationToken
            );
            ...
            if (!string.IsNullOrWhiteSpace(headersText))
            {
                ...
                foreach (var header in headersQuery)
                {
                    ...
                    var headerValue = await _expressionEvaluator.EvaluateAsync(
